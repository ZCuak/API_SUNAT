...
// Asignar valores opcionales a la venta
$venta['fecha_vencimiento'] = $venta['fecha_vencimiento'] ?? null;
$venta['total_exonerada']   = $venta['total_exonerada'] ?? null;
$venta['total_inafecta']    = $venta['total_inafecta'] ?? null;

// Extraer los detalles de los items
$detalle = [];
 ...
 */
function crear_xml(string $nombre, array $empresa, array $cliente, array $venta, array $detalle, array $cuotas): void {
 ...
 */
function firmar_xml(string $name_file, int $entorno, string $baja = ''): void {
...
/**
 * Función para generar el contenido XML
 */
function desarrollo_xml(array $empresa, array $cliente, array $venta, array $detalles, array $cuotas): string {
    $total_igv = $venta['total_igv'] ?? 0.0;
    $total_gravada = $venta['total_gravada'] ?? 0.0;
    $total_exonerada = $venta['total_exonerada'] ?? 0.0;
    $total_inafecta = $venta['total_inafecta'] ?? 0.0;
    ...
    // Agregar los campos de firma y datos de la empresa
    $xml .= '<cac:Signature>
    ...
    $xml .= '<cac:AccountingCustomerParty>
                <cac:Party>
                    <cac:PartyIdentification>
                        <cbc:ID schemeID="' . $cliente['codigo_tipo_entidad'] . '" schemeName="Documento de Identidad" schemeAgencyName="PE:SUNAT">' . $cliente['numero_documento'] . '</cbc:ID>
                    </cac:PartyIdentification>
                    <cac:PartyLegalEntity>
    ...
    // Forma de pago
    if (in_array($venta['tipo_documento_codigo'], ['01', '03'])) {
        if ($venta['forma_pago_id'] == 1) {
            $xml .= '<cac:PaymentTerms>
                        <cbc:ID>FormaPago</cbc:ID>
                        <cbc:PaymentMeansID>Contado</cbc:PaymentMeansID>
                    </cac:PaymentTerms>';
        } elseif ($venta['forma_pago_id'] == 2) {
    ...
    // Totales de impuestos y pagos
    $xml .= '<cac:TaxTotal>
                <cbc:TaxAmount currencyID="' . $codigo_moneda . '">' . $total_igv . '</cbc:TaxAmount>';
    if ($total_gravada > 0) {
        $xml .= '<cac:TaxSubtotal>
                    <cbc:TaxableAmount currencyID="' . $codigo_moneda . '">' . $total_gravada . '</cbc:TaxableAmount>
                    <cbc:TaxAmount currencyID="' . $codigo_moneda . '">' . $total_igv . '</cbc:TaxAmount>
                    <cac:TaxCategory>
                        <cac:TaxScheme>
                            <cbc:ID>1000</cbc:ID>
    ...
    if ($total_exonerada > 0) {
        $xml .= '<cac:TaxSubtotal>
                    <cbc:TaxableAmount currencyID="' . $codigo_moneda . '">' . $total_exonerada . '</cbc:TaxableAmount>
    ...
    if ($total_inafecta > 0) {
        $xml .= '<cac:TaxSubtotal>
                    <cbc:TaxableAmount currencyID="' . $codigo_moneda . '">' . $total_inafecta . '</cbc:TaxableAmount>
                    <cbc:TaxAmount currencyID="' . $codigo_moneda . '">0.00</cbc:TaxAmount>
                    <cac:TaxCategory>
                        <cac:TaxScheme>
                            <cbc:ID>9998</cbc:ID>
...
/**
 * Función para devolver la moneda y su descripción
 */
function moneda(int $moneda_id): array {
    $codigo_moneda = 'PEN';
    $descripcion = 'soles';
    switch ($moneda_id) {
        case 2:
    ...
    return [$codigo_moneda, $descripcion];
}

/**
 * Función para realizar el envío a SUNAT
 */
function ws_sunat(array $empresa, string $nombre_archivo): void {
    ...
    $respuesta_codigo = $info['codigo'] ?? '';
    $respuesta_mensaje = $info['message'] ?? '';
