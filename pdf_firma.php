...
if (!isset($_POST['datosJSON'])) {
    die(json_encode(['error' => 'No se recibieron datos JSON']));
}
...
if (!$array_llegada) {
    die(json_encode(['error' => 'El JSON es inválido']));
}
...
function crear_pdf(array $empresa, array $cliente, array $venta, array $detalle, string $nombre): void {
    try {
        ...
        $pdf->Cell(10, 10, number_format(($detalle[0]['cantidad'] * ($detalle[0]['precio_base'] * 1.18)), 2), 0, 0, 'R');
        $pdf->Ln(4);
        $pdf->Cell(12, 10, number_format($detalle[0]['precio_base'] * 1.18, 2) . " x " . $detalle[0]['cantidad'], 0, 0, 'L');
        $pdf->Ln(4);
        ...
        // Guardar el PDF
        $pdf->Output('files/facturacion_electronica/PDF_FIRMA/' . $nombre . '.pdf', 'F');
    } catch (Exception $e) {
...
/**
 * Función para obtener la firma del archivo XML
 */
function getFirma(string $NomArch): string {
    try {
        $ruta = 'files/facturacion_electronica/FIRMA/';

        if (!file_exists($ruta . $NomArch . '.xml')) {
            throw new Exception("El archivo XML no se encuentra: " . $NomArch . '.xml');
        }

        $xml = simplexml_load_file($ruta . $NomArch . '.xml');
        if ($xml === false) {
            throw new Exception("Error al cargar el archivo XML: " . $NomArch . '.xml');
        }

        $response = null;
        foreach ($xml->xpath('//ds:DigestValue') as $digestValue) {
            $response = (string)$digestValue;
        }

        if (is_null($response)) {
            throw new Exception("No se encontró el valor DigestValue en el archivo XML: " . $NomArch . '.xml');
        }

        return $response;
    } catch (Exception $e) {
        echo json_encode(['error' => 'Error al obtener la firma: ' . $e->getMessage()]);
        exit;
    }
}
